...
@st.cache_data
def load_ML():
    # Matrice numpy (int8, ~40MB) contenant les features encodées pour le KNN (sans les colonnes texte)
    # Beaucoup + léger qu'un DF pandas (~330MB)
    lien = "https://huggingface.co/datasets/Elisa-Guerin/dfimdbML3_V2/resolve/main/X_matrix.npy"
...
# on indente tout notre sit a l'intérieur des conditions de log sinon le site s'affichera sans autentification)
    ...
    # Appels des fonctions de chargement — mis en cache au premier chargement
    df_affichage = load_affichage()
    X = load_ML()
    model4 = load_model()
